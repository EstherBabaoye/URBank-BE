...
<body style="margin:0;padding:0;font-family:Arial,sans-serif;background-color:#f4f4f4;">
  <div style="max-width:600px;margin:0 auto;background:white;border-radius:8px;overflow:hidden;">
    ...
    <!-- Body -->
    <div style="padding:30px; color:#051d40;">
      <p>Hello {{ $fullName }},</p>

      <p>Your <strong>{{ $subType }}</strong> card has been <strong>approved</strong>.</p>

      <p style="line-height:1.6;">
        <strong>Card Number:</strong> {{ $cardNumber }}<br>
        <strong>CVV:</strong> {{ $cvv }}<br>
        <strong>Default ATM PIN:</strong> {{ $atm_pin }}<br>
        <strong>Expiry Date:</strong> {{ $expiryDate }}<br>
        <strong>Linked Account Number:</strong> {{ $accountNumber }}
      </p>

      <p>You may pick up your card at any URBank location or request for delivery.</p>
